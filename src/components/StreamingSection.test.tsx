...
it('groups provider prices by quality and labels access', () => {
...
it('shows one provider row and one equal price with separately clickable format URLs',()=>{
 ...
 expect(within(row).getAllByText('Amazon')).toHaveLength(1);
 expect(within(row).getAllByText('$19.98')).toHaveLength(1);
 expect(row.querySelectorAll('img')).toHaveLength(1);
 formats.forEach((format,index)=>expect(within(row).getByRole('link',{name:`Amazon rent ${format}: $19.98`}).getAttribute('href')).toBe(`https://example.com/${index}`));
});
