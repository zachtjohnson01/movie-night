...
export default function CreatorCatalog({ pool, library, familySlug, children, onOpenChange, modern = false }: {
  ...
  useEffect(() => {
    ...
    element?.showModal();
    return () => { element?.close(); previousFocus?.focus({preventScroll:true}); window.scrollTo(scrollX, scrollY); };
  }, [isOpen]);
  function back() {
  ...
  return <CreatorCatalogContext.Provider value={selection => setLayers(current => [...current, {selection}])}>
    {children}
    {layer && <dialog ref={dialog} aria-label={layer.movie ? `${layer.movie.title} details` : `More films: ${layer.selection.name}`} onCancel={e => { e.preventDefault(); back(); }} className="fixed inset-0 m-auto w-full max-w-xl max-h-[94dvh] rounded-t-3xl sm:rounded-3xl border border-ink-700 bg-ink-950 p-0 text-ink-100 backdrop:bg-black/70 overflow-y-auto">
      <div className="sticky top-0 z-50 flex items-start justify-between gap-3 bg-ink-950 px-5 pt-5 pb-2">
